 ...
 * use the {@link YerushalmiPerekYomiCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class PerekYomiCalculator {
  private static readonly perekYomiStartDate: DateTime = DateTime.fromObject({
    year: 2006,
    month: Calendar.JUNE + 1,
    day: 3,
  });

  private static readonly perekYomiJulianStartDay: number = PerekYomiCalculator.getJulianDay(PerekYomiCalculator.perekYomiStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = PerekYomiCalculator.getJulianDay(PerekYomiCalculator.shekalimChangeDate);
   ...
   */
  public static getPerekYomiChapter(calendar: JewishCalendar): number {
    /*
    ...
    cycleNo = 1 + ((julianDay - PerekYomiCalculator.perekYomiJulianStartDay) % 525);
