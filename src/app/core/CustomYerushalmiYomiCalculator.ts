...
import { Calendar, JewishCalendar, Daf } from 'kosher-zmanim';
 ...
 */
export class CustomYerushalmiYomiCalculator {
    ...
    public static getDafYomiYerushalmi(jewishCalendar: JewishCalendar): Daf {
      ...
      if (requested < CustomYerushalmiYomiCalculator.DAF_YOMI_START_DAY) {
        // TODO: should we return a null or throw an ?
        throw new Error(`IllegalArgumentException: ${requested} is prior to organized Daf Yomi Yerushlmi cycles that started on ${CustomYerushalmiYomiCalculator.DAF_YOMI_START_DAY}`);
      }
  
      // Start to calculate current cycle. Initialize the start day
      nextCycle = CustomYerushalmiYomiCalculator.DAF_YOMI_START_DAY;
      ...
      while (requested > nextCycle) {
        ...
        nextCycle = nextCycle.plus({ days: CustomYerushalmiYomiCalculator.WHOLE_SHAS_DAFS })
          .plus({ days: CustomYerushalmiYomiCalculator.getNumOfSpecialDays(prevCycle, nextCycle) });
      }
      ...
      const specialDays: number = CustomYerushalmiYomiCalculator.getNumOfSpecialDays(prevCycle, requested);
      let total: number = dafNo - specialDays;
  
      // Finally find the daf.
      for (let i: number = 0; i < CustomYerushalmiYomiCalculator.BLATT_PER_MASECHTA.length; i++) {
        if (total <= CustomYerushalmiYomiCalculator.BLATT_PER_MASECHTA[i]) {
          dafYomi = new Daf(masechta, total + 1);
          break;
        }
        total -= CustomYerushalmiYomiCalculator.BLATT_PER_MASECHTA[i];
        masechta++;
      }
