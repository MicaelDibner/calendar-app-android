 ...
 * use the {@link YerushalmiLikutyMoranCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class LikutyMoranCalculator {
  private static readonly likutyMoranStartDate: DateTime = DateTime.fromObject({
    year: 1998,
    month: Calendar.MAY + 1,
    day: 17,
  });

  private static readonly likutyMoranStartDay: number = LikutyMoranCalculator.getJulianDay(LikutyMoranCalculator.likutyMoranStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = LikutyMoranCalculator.getJulianDay(LikutyMoranCalculator.shekalimChangeDate);
   ...
   */
  public static getLikutyMoranChapter(calendar: JewishCalendar): number {
    /*
    ...
    cycleNo = 1 + ((julianDay - LikutyMoranCalculator.likutyMoranStartDay) % 331);
