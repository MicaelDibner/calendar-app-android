 ...
 * use the {@link YerushalmiLikutyHalhotCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class LikutyHalhotCalculator {
  private static readonly likutyHalhotStartDate: DateTime = DateTime.fromObject({
    year: 2004,
    month: Calendar.DECEMBER + 1,
    day: 22,
  });

  private static readonly likutyHalhotStartDay: number = LikutyHalhotCalculator.getJulianDay(LikutyHalhotCalculator.likutyHalhotStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = LikutyHalhotCalculator.getJulianDay(LikutyHalhotCalculator.shekalimChangeDate);
   ...
   */
  public static getLikutyHalhotChapter(calendar: JewishCalendar): number {
    /*
