 ...
 * use the {@link YerushalmiHalahaYomitCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class HalahaYomitCalculator {
  private static readonly halahaYomitStartDate: DateTime = DateTime.fromObject({
    year: 1950,
    month: Calendar.MARCH + 1,
    day: 17,
  });

  private static readonly halahaYomitStartDay: number = HalahaYomitCalculator.getJulianDay(HalahaYomitCalculator.halahaYomitStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = HalahaYomitCalculator.getJulianDay(HalahaYomitCalculator.shekalimChangeDate);
   ...
   */
  public static getHalahaYomitChapter(calendar: JewishCalendar): number {
    /*
    ...
    cycleNo = 1 + ((julianDay - HalahaYomitCalculator.halahaYomitStartDay) % 1613);
