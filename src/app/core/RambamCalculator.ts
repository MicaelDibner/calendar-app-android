 ...
 * use the {@link YerushalmiRambamCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class RambamCalculator {
  private static readonly rambamStartDate: DateTime = DateTime.fromObject({
    year: 1984,
    month: Calendar.APRIL + 1,
    day: 29,
  });

  private static readonly rambamJulianStartDay: number = RambamCalculator.getJulianDay(RambamCalculator.rambamStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = RambamCalculator.getJulianDay(RambamCalculator.shekalimChangeDate);
   ...
   */
  public static getRambam1Perek(calendar: JewishCalendar): number {
    /*
    ...
    cycleNo = 1 + ((julianDay - RambamCalculator.rambamJulianStartDay) % 1017);
  ...
  public static getRambam3Perek(calendar: JewishCalendar): number {
    /*
     * The number of daf per masechta. Since the number of blatt in Shekalim changed on the 8th Daf Yomi cycle
     * beginning on June 24, 1975 from 13 to 22, the actual calculation for blattPerMasechta[4] will later be
     * adjusted based on the cycle.
    ...
    const julianDay: number = this.getJulianDay(date);
    let cycleNo: number = 0;
    cycleNo = 1 + ((julianDay - RambamCalculator.rambamJulianStartDay) % 339);
