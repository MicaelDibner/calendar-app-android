 ...
 * use the {@link YerushalmiTanahCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class TanahCalculator {
  private static readonly tanahStartDate: DateTime = DateTime.fromObject({
    year: 2007,
    month: Calendar.NOVEMBER + 1,
    day: 1,
  });

  private static readonly tanahJulianStartDay: number = TanahCalculator.getJulianDay(TanahCalculator.tanahStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = TanahCalculator.getJulianDay(TanahCalculator.shekalimChangeDate);
   ...
   */
  public static getTanahChapter(calendar: JewishCalendar): number {
    /*
    ...
    cycleNo = 1 + ((julianDay - TanahCalculator.tanahJulianStartDay) % 742);
