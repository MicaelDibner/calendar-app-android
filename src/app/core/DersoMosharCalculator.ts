 ...
 * use the {@link YerushalmiDersoMosharCalculator}. The library may cover Mishna Yomi etc. at some point in the future.
 *
 * @author &copy; Bob Newell (original C code)
 * @author &copy; Eliyahu Hershfeld 2011 - 2019
 * @version 0.0.1
 */
export class DersoMosharCalculator {
  private static readonly dersoMosharStartDate: DateTime = DateTime.fromObject({
    year: 2016,
    month: Calendar.APRIL + 1,
    day: 10,
  });

  private static readonly dersoMosharStartDay: number = DersoMosharCalculator.getJulianDay(DersoMosharCalculator.dersoMosharStartDate);
  private static readonly shekalimChangeDate: DateTime = DateTime.fromObject({ year: 1975, month: Calendar.JUNE + 1, day: 24 });
  private static readonly shekalimJulianChangeDay: number = DersoMosharCalculator.getJulianDay(DersoMosharCalculator.shekalimChangeDate);
   ...
   */
  public static getDersoMocharChapter(calendar: JewishCalendar): number {
    /*
    ...
    const jsDate = new Date(date.ts);
    console.log(jsDate.getDay());
    const julianDay: number = this.getJulianDay(date);
    let cycleNo: number = 0;
    if(jsDate.getDay() === 6) { return 0; }
    cycleNo = 1 + ((julianDay - DersoMosharCalculator.dersoMosharStartDay) % 2142);
    // console.log(cycleNo  + ' whole cycle');
    let daysAfterShabbat = cycleNo % 7;
    // console.log(daysAfterShabbat + ' days after shabbat');
    let numberOfShabbats = (cycleNo - daysAfterShabbat) / 7;
    // console.log(numberOfShabbats + ' pass shabbats');
    cycleNo = cycleNo - numberOfShabbats;
